...
//    }
    ...
    host("Schedule Azure DEV Deployment") {
        kotlinScript { api ->
            val deployVersion = "Aardvark-v0.1." + api.executionNumber() + "s"
...
job("Azure DEV Deployment") {
    ...
    val zipFile = "doky-solr.tgz"
    container(displayName = "Archive", image = "ubuntu:latest") {
        shellScript {
            content = """
                   ...
                   tar -cvzf $zipFile ./
                   cd docker
                   cp $zipFile ${'$'}JB_SPACE_FILE_SHARE_PATH/$sharedBuildPath
...
//    container(displayName = "Push to azure", "mcr.microsoft.com/azure-cli") {
